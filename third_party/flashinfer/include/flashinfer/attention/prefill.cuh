...
__device__ __forceinline__ void SinglePrefillWithKVCacheDevice(
  ...
  if constexpr (std::is_same_v<DTypeQ, nv_bfloat16>) {
    ...
    /* Qwen3.8's device graph owns its causal position in a persistent CUDA
     * scalar.  Params::get_kv_len() is already the public per-item accessor;
     * using it here keeps the generated single-prefill kernel capture-safe
     * without changing any host dispatch geometry. */
    const uint32_t kv_len = params.get_kv_len(0u);
