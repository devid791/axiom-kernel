/* Resident Qwen3.8 Gated DeltaNet state/weight gate. */
...
int fail(const char *what, int rc = AXIOM_OK) {
    std::fprintf(stderr, "axiom-qwen38-gdn-probe: %s%s%s\n", what,
    ...
    for (uint32_t column = 0u; column < kBatch; ++column) {
        for (uint32_t i = 0u; i < kHidden; ++i) {
            (*values)[static_cast<size_t>(column) * kHidden + i] =
                    0.41f * std::sin(0.0017f * static_cast<float>(i) + 0.13f * column) +
                    0.23f * std::cos(0.0083f * static_cast<float>(i) - 0.071f * column);
        }
    }
}
...
int main(int argc, char **argv) {
    if (argc < 2 || argc > 3) {
        std::fprintf(stderr, "usage: %s MODEL_DIR [GDN_LAYER]\n", argv[0]);
        return 2;
    }
    char *end = nullptr;
    const uint32_t layer_index = argc == 3
            ? static_cast<uint32_t>(std::strtoul(argv[2], &end, 10)) : 0u;
    if ((argc == 3 && (end == argv[2] || *end != '\0')) || (layer_index % 4u) == 3u) {
        return fail("GDN layer must be an integer not congruent to 3 modulo 4");
    }
    ...
    model_config.placement.abi_version = AXIOM_PLACEMENT_LOCAL_DEVICE;
    ...
    axiom_qwen38_gdn_layer *layer = nullptr;
    rc = axiom_qwen38_gdn_layer_load(model, runtime, 0, layer_index, &layer);
    axiom_model_close(model);
    if (rc != AXIOM_OK) {
        axiom_runtime_destroy(runtime);
        return fail("Gated DeltaNet load", rc);
    }
    ...
    rc = axiom_qwen38_gdn_layer_forward_f32_device(layer, d_input, d_output, nullptr);
    ...
    if (rc == AXIOM_OK) rc = axiom_qwen38_gdn_layer_forward_f32_device(layer, d_input, d_output, nullptr);
    if (rc == AXIOM_OK && cudaDeviceSynchronize() != cudaSuccess) rc = AXIOM_ERR_CUDA;
    if (rc == AXIOM_OK && cudaMemcpy(second.data(), d_output, bytes, cudaMemcpyDeviceToHost) != cudaSuccess) {
        rc = AXIOM_ERR_CUDA;
    }
    if (rc == AXIOM_OK) rc = axiom_qwen38_gdn_layer_reset(layer);
    if (rc == AXIOM_OK) rc = axiom_qwen38_gdn_layer_forward_f32_device(layer, d_input, d_output, nullptr);
    if (rc == AXIOM_OK && cudaDeviceSynchronize() != cudaSuccess) rc = AXIOM_ERR_CUDA;
    ...
    const bool pass = finite && state_delta > 0.0f && reset_delta == 0.0f;
    std::printf("{\"status\":\"%s\",\"source\":\"unsloth/Qwen3.8-27B-NVFP4\","
                "\"backend\":\"axiom-native-qwen38-gdn\",\"layer\":%u,\"batch\":8,"
                "\"device_bytes\":%llu,\"state_delta\":%.9g,\"reset_delta\":%.9g}\n",
                pass ? "pass" : "fail", layer_index,
                static_cast<unsigned long long>(axiom_qwen38_gdn_layer_device_bytes(layer)),
